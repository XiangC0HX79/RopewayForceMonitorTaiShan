package forceMonitor.model
{
	...
	public class RopewayAlarmDealProxy extends WebServiceProxy implements IProxy
	{
		...
		{			
			sendNotification(ForceMonitorFacade.NOTIFY_MAIN_LOADING_SHOW,"正在处理报警信息...");
		...
		private function onUpdateAlarmDeal(event:ResultEvent):void
		{
			...
			if(event.result)
			{
				...
				sendNotification(ForceMonitorFacade.NOTIFY_ALERT_INFO,"报警处置成功。");
			}
			else
			{
				sendNotification(ForceMonitorFacade.NOTIFY_ALERT_INFO,"报警处置失败。");				
			}
		}		
	}
}