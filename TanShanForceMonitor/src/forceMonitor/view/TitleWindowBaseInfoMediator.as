package forceMonitor.view
{
	...
	import forceMonitor.view.components.TitleWindowBaseInfo;
	...
	public class TitleWindowBaseInfoMediator extends Mediator implements IMediator
	{
		...
		private function onBaseInfoNew(event:Event):void
		{
			...
			if(ropewayCarId == "")
			{
				sendNotification(ForceMonitorFacade.NOTIFY_ALERT_ALARM,"请输入吊箱编号！");
				return;
			}
			...
			if(ropewayId == "")
			{
				sendNotification(ForceMonitorFacade.NOTIFY_ALERT_ALARM,"请输入抱索器编号！");
				return;
			}
			...
			if(ropewayRFID == "")
			{
				sendNotification(ForceMonitorFacade.NOTIFY_ALERT_ALARM,"请输入RFID编号！");
				return;
			}
			...
			for each(var r:RopewayBaseinfoVO in _ropewayBaseinfoProxy.colBaseinfo)
			{
				...
				{
					sendNotification(ForceMonitorFacade.NOTIFY_ALERT_ALARM,"吊箱编号已存在！");
					return;					
				}
				
				if(r.ropewayId == ropewayId)
				{
					sendNotification(ForceMonitorFacade.NOTIFY_ALERT_ALARM,"抱索器编号已存在！");
					return;					
				}
				
				if(r.ropewayRFID == ropewayRFID)
				{
					sendNotification(ForceMonitorFacade.NOTIFY_ALERT_ALARM,"RFID编号已存在！");
					return;					
				}
			}
		...
		{
			return [
				ForceMonitorFacade.NOTIFY_ROPEWAY_BASEINFO_NEW,				
				ForceMonitorFacade.NOTIFY_ROPEWAY_BASEINFO_EDIT
			];
		}
		...
		override public function handleNotification(notification:INotification):void
		{
			...
			{
				case ForceMonitorFacade.NOTIFY_ROPEWAY_BASEINFO_NEW:
				...
				case ForceMonitorFacade.NOTIFY_ROPEWAY_BASEINFO_EDIT:
